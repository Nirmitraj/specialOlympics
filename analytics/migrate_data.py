...
class Command():
    ...
    def calculate_student_nonwhite_population(self, *args):
        ...
        for population in args:
            if population is not None and population != '':
                print("Non white population", population)

                nonwhite_population += float(population)
        print(nonwhite_population)
    ...
    def handle(self, *args, **kwargs):
        datafile = os.path.join(settings.BASE_DIR,'excel_data/Y9_UCS_Liaison_Survey_Longitudinal.csv') 
        ...
        with open(datafile, newline='', encoding='utf-8-sig') as csvfile:
            ...
            for row in reader:
                ...
                implementation_level= None, #self.clean_str_implementation_level_null(row['Y9_ImplementationLevel']),#12345
                ...
                survey_taken=self.number_bool(row['Y9_data']),#all
                unified_sports_component= None, #self.number_bool(row['Y9_InclusiveSportsComponent']),#1235
                youth_leadership_component= None,  #self.number_bool(row['Y9_YouthLeadershipComponent']),#1235
                whole_school_component=None, #self.number_bool(row['Y9_WholeSchoolComponent']),#1235
                zipcode=self.clean_str_zip_null(row['UD_ZIP']),#all
                survey_taken_year=2017,

                sports_sports_teams =self.clean_str_null(row['Y9_b1.a_dichot']),#13
                sports_unified_PE=self.clean_str_null(row['Y9_b1.b_dichot']),#13
                ...
                increasing_confidence_of_students_with_idd=self.clean_str_null(row['Y9_h1.d_recode']),#1
                ...
                increasing_participation_of_students_without_idd=self.clean_str_null(row['Y9_h1.b_recode']),#1
